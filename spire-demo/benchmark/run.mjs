...
const selected=(fresh?freshCases:cases).filter(c=>process.argv.includes('--scored-only')?c.check:process.argv.includes('--review-only')?!c.check:true);
const policies=process.argv.includes('--comparison')?['simple','comparison']:['current','simple'];
const dry=process.argv.includes('--dry-run');
const stamp=new Date().toISOString().replaceAll(':','-');
const dir=new URL(`../../.private/spire-benchmark/${stamp}/`,import.meta.url);
await mkdir(dir,{recursive:true});
const sources=[...(await readdir(new URL('../',import.meta.url))).filter(p=>p.endsWith('.mjs')&&!p.endsWith('.test.mjs')), 'benchmark/simple.mjs','benchmark/action-comparison.mjs','benchmark/suite.mjs','benchmark/run.mjs','benchmark/fresh-suite.mjs'];
...
for(let repeat=0;repeat<repeats;repeat++)for(let i=0;i<selected.length;i++){
 ...
 for(const policy of (i+repeat)%2?[...policies].reverse():policies){
  ...
  try{
   const result=await ({current:deliberate,simple:simpleDeliberate,comparison:comparisonDeliberate}[policy])({state,candidates,recent,ask:async payload=>{
   ...
   row={...row,choice:chosen.id,label:chosen.label,command:chosen.command,grade:(fresh?gradeFresh:grade)(test,chosen),usage:result.usage,calls:calls.length};
