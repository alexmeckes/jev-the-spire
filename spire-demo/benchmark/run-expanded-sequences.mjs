...
import {sequenceCases as baseCases,stepLab,labActions} from './sequence-lab.mjs';
...
const repeats=1,dir=new URL(`../../.private/spire-benchmark/expanded-sequences-${new Date().toISOString().replaceAll(':','-')}/`,import.meta.url);
...
const config=await readFile(new URL('../../.private/typesafe.cfg',import.meta.url),'utf8');
...
for(let repeat=0;repeat<repeats;repeat++)for(let i=0;i<sequenceCases.length;i++){
 ...
 for(const policy of (i+repeat)%2?['comparison','simple','current']:['current','simple','comparison']){
  ...
  const row={fixture:fixture.id,split:fixture.split,oracleHp:optima[fixture.id],policy,repeat,terminal,error,hp:state.player.hp,enemyHp:state.battle.enemies[0].hp,actions:steps.map(x=>x.chosen.plan?.[0]?.label??x.chosen.label),calls:calls.length,inputTokens:calls.reduce((n,c)=>n+(c.response.usage?.input_tokens??0),0)};
  results.push(row);
