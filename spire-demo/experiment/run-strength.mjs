...
import {strengthDeliberate} from './strength-support.mjs';
...
const dir=new URL('.private/spire-strength/'+new Date().toISOString().replaceAll(':','-')+'/',root);await mkdir(dir,{recursive:true});
await writeFile(new URL('inputs.json',dir),JSON.stringify(samples));
for(const file of ['strength-support.mjs','run-strength.mjs'])await writeFile(new URL(file,dir),await readFile(new URL(file,import.meta.url)));
...
for(let repeat=0;repeat<2;repeat++)for(const [i,s] of samples.entries())for(const policy of (i+repeat)%2?['strength','current']:['current','strength']){
 ...
 try{
  ...
  const result=policy==='current'?await deliberate(opts):await strengthDeliberate(opts);
