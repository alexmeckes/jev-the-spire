...
import {marginalDeliberate} from './reward-marginal.mjs';
...
for(const file of ['reward-marginal.mjs','run-marginal.mjs'])await writeFile(new URL(file,dir),await readFile(new URL(file,import.meta.url)));
...
for(let repeat=0;repeat<2;repeat++)for(const [i,s] of samples.entries())for(const policy of (i+repeat)%2?['marginal','current']:['current','marginal']){
