...
import {decisionCandidates,withRageReorders} from '../planner.mjs';
...
const dir=new URL('.private/spire-rage-reorder/'+new Date().toISOString().replaceAll(':','-')+'/',root);await mkdir(dir,{recursive:true});
await writeFile(new URL('inputs.json',dir),JSON.stringify(samples));
await writeFile(new URL('planner.mjs',dir),await readFile(new URL('../planner.mjs',import.meta.url)));
for(const file of ['run-rage-reorder.mjs'])await writeFile(new URL(file,dir),await readFile(new URL(file,import.meta.url)));
...
for(let repeat=0;repeat<2;repeat++)for(const [i,s] of samples.entries())for(const policy of (i+repeat)%2?['reorder','current']:['current','reorder']){
 const calls=[],start=Date.now();let row={id:s.id,policy,repeat,recorded:s.recorded,eligible:s.eligible};
 try{
  const candidates=policy==='reorder'?withRageReorders(s.state,s.candidates):s.candidates;
  const opts={state:s.state,candidates,recent:s.recent,ask:async payload=>{
  ...
  const result=await deliberate(opts);
  const c=candidates.find(c=>c.id===result.answers?.move?.choice);if(!c)throw Error('Invalid choice');
  row={...row,label:c.label,command:c.command,forecast:c.forecast,reorderedFrom:c.reorderedFrom,candidateCount:candidates.length,grade:s.test?(s.id.startsWith('fresh-')?gradeFresh:grade)(s.test,c):{status:'review-only'}};
