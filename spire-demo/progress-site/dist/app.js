...
function runBuild(r){
 const b=r.build;
 if(!b)return '<details class="run-extra run-build"><summary>View build</summary><p>Build not recorded.</p></details>';
 const cards=b.deck.map(c=>`<li><span>${escapeHtml(c.name)}${c.upgrades?`<span class="card-upgrade">+${c.upgrades>1?c.upgrades:''}</span>`:''}${c.enchantment?`<small>${escapeHtml(c.enchantment)}${c.enchantmentAmount!=null?` · ${c.enchantmentAmount}`:''}</small>`:''}</span><span class="build-count" aria-label="${c.count} copies">×${c.count}</span></li>`).join('');
 const list=items=>items.map(name=>`<li>${escapeHtml(name)}</li>`).join('');
 return `<details class="run-extra run-build"><summary>View build <span>${b.cardCount} cards · ${b.relics.length} relics</span></summary><div class="run-build-body"><h4>Deck at end of run</h4><p class="build-note">+ marks an upgrade.</p><ul class="run-deck">${cards}</ul><h4>Relics <span>${b.relics.length}</span></h4><ul>${list(b.relics)}</ul><h4>Potions left</h4>${b.potions===null?'<p>Not recorded.</p>':b.potions.length?`<ul>${list(b.potions)}</ul>`:'<p>None.</p>'}</div></details>`;
}
