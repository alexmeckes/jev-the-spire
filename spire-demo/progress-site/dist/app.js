...
$('#milestones').innerHTML=milestones.map(m=>{const p=(m.planner?D.plannerVersions:D.reviewVersions)[m.key];return `<button class="milestone" data-run="${p.run}"><div class="meta"><span>RUN ${p.run}</span><span class="badge">${m.tag}</span></div><h3>${m.title}</h3><p>${m.text}</p><div class="evidence">First logged: ${date(p.time)} ET</div></button>`}).join('')+`<article class="milestone"><div class="meta"><span>SEP 22 · REPORT DATE</span><span class="badge">Reliability</span></div><h3>Shorten oversized requests</h3><p>Removed repeated card-order details to stop request-size errors. The available choices and warnings stayed the same.</p><div class="evidence">Overnight report · 6:01 PM ET</div></article>`;
const extra=[{n:134,title:'Try Luna as an adviser',text:'Luna Max reviewed two choices before Jev made the final call. It was too slow, so we stopped the trial and returned to Jev alone.',when:'Sep 21, 10:06 PM ET · first logged use',tag:'Stopped experiment'},{n:152,title:'Fix repeated Strike damage',text:'Fixed the damage estimate for plain Strike repeated by Replay, including effects that apply on each hit. Checked against recorded game actions.',when:'Sep 22, 8:19 AM ET · deployment report',tag:'Calculation fix'}];
