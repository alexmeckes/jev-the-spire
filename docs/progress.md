...
See [every run in CSV](../spire-demo/progress-site/dist/run-tokens.csv). All 58,588 decision entries contain input usage. Each decision's `usage` already aggregates its assessment, review, and any extra review passes, so those are counted once. Logged previews, cancellations and stale decisions still used tokens and are included. Successful calls in a pipeline that later failed or was interrupted may not have a decision entry and may be missing. These are recorded usage totals, not a provider billing statement. Offline API evaluations and the brief Luna adviser's tokens are excluded; Jev calls in that assisted run remain included. No dollar estimate is derived here.
